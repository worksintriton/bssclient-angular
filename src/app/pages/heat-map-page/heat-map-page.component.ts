...
import {CHART_COLOR_1, CHART_COLOR_5, CHART_TEXT_COLOR} from '../../utils/colors';
...
export class HeatMapPageComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('heatMapChart')
  heatMapChartElementRef: ElementRef;
  heatMapChart;
  // Model for heatmap
  heatmapChartOption = this.createOption();
  ...
  ngOnInit() {
    this.heatMapChart = echarts.init(this.heatMapChartElementRef.nativeElement) ;
    this.heatMapChart.setOption(this.heatmapChartOption);
  ...
  /**
   * Mock method for heat map data generation
   */
  createOption() {
    ...
    const option = {
      ...
      visualMap: {
        ...
        bottom: '15%',
        inRange: {
          color: [CHART_COLOR_5, CHART_COLOR_1]
        },
        textStyle: {
          color: CHART_TEXT_COLOR
        }
