...
import {
  CHART_COLOR_1, CHART_COLOR_1_WITH_OPACITY, CHART_COLOR_2, CHART_COLOR_2_WITH_OPACITY, CHART_COLOR_3, CHART_COLOR_4,
  CHART_COLOR_5,
  CHART_TEXT_COLOR
} from '../../utils/colors';
...
export class BubbleChartPageComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('simpleBubbleChart')
  simpleBubbleChartElementRef: ElementRef;
  simpleBubbleChart;

  @ViewChild('singleAxisBubbleChart')
  singleAxisBubbleChartElementRef: ElementRef;
  singleAxisBubbleChart;
  ...
  simpleBubbleChartOption = {
    ...
    series: [{
      name: 'Data 1',
      data: this.simpleBubbleChartData[0],
      type: 'scatter',
      ...
      name: 'Data 2',
      data: this.simpleBubbleChartData[1],
      type: 'scatter',
      symbolSize: function (data) {
        return Math.sqrt(data[2]) / 5e2;
      },
      ...
      itemStyle: {
        normal: {
          shadowBlur: 10,
          shadowColor: CHART_COLOR_1_WITH_OPACITY,
          shadowOffsetY: 5,
          color: new echarts.graphic.RadialGradient(0.4, 0.3, 1, [{
            offset: 0,
            color: CHART_COLOR_1
  ...
  // Single axis bubble chart model
  ...
  ngOnInit() {
    this.simpleBubbleChart = echarts.init(this.simpleBubbleChartElementRef.nativeElement);
    this.simpleBubbleChart.setOption(this.simpleBubbleChartOption);

    this.singleAxisBubbleChart = echarts.init(this.singleAxisBubbleChartElementRef.nativeElement);
    this.singleAxisBubbleChart.setOption(this.singleAxisBubbleChartOption);

    this.resizeService.resizeInformer$.pipe(takeUntil(this.ngUnsubscribe)).subscribe(() => {
      this.simpleBubbleChart.resize();
      this.singleAxisBubbleChart.resize();
    })
  }
  ...
  createSingleAxisBubbleChartOption() {
    ...
    const colors = [CHART_COLOR_1, CHART_COLOR_2, CHART_COLOR_3, CHART_COLOR_4, CHART_COLOR_5];
